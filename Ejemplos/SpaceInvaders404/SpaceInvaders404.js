...
var SpaceInvaders404 = function() {
    ...
    this.Buffer404          = null;
    this.Mouse              = { X : 0, Y : 0 };
    this.BloquearControles  = true; // Si es false, la estructura mouse recibira las coordenadas del ratón, si es true la estrucutra mouse no se actualizará y podrá ser utilizada para una animación
...
//        HUD     : false,                // Ayuda visual con las barras de la X, Y del mouse, y el ángulo de disparo
        ...
        Invulnerable : true,            // Jugador invulnerable
...
//            DisparosPorSegundo    : 3
    ...
    this.TipoEnemigo = { // imgs de 60x30
    ...
    this.Enemigo = function(X, Y, Tipo, PosLinea, Linea) {
        this.X                  = X;
        this.Y                  = Y; 
        this.Tipo               = Tipo;
    ...
    this.SiguienteOleadaEnemigos = function() {
        if (this.Oleada === null) {
...
//        this.MuertoMS = 0;  // variable para la invulnerabilidad despues de morir
    ...
    this.Disparo = function(X, Y, Angulo, Tipo) {
        this.X          = X;        // x inicial
        this.Y          = Y;        // y inicial
...
//        var Fondo = HexToRgb("#EA504E");
        ...
        for (var i = this.Disparos.length - 1; i > -1; i--) {
            ...
            var c = Math.cos(this.Disparos[i].Angulo * (Math.PI / 180));
            // Avanzo los disparos
            this.Disparos[i].Distancia += this.Opciones.Tanque.DisparosVelocidad / 3;
            var ux = this.Disparos[i].X - (this.Disparos[i].Distancia * c);
            var uy = this.Disparos[i].Y - (this.Disparos[i].Distancia * s);
            this.Disparos[i].Distancia += (this.Opciones.Tanque.DisparosVelocidad / 3) *2;
...
//                if (this.Enemigos.length > 0) {
 //                   if (y - this.Opciones.Tanque.DisparosVelocidad <= this.Enemigos[this.Enemigos.length - 1].Y) {
   ...
   //                     }
    ...
    this.CrearExplosion = function(X, Y, Rad) {
    ...
    this.PintarExplosiones = function() {
    ...
    this.CrearTexto = function(nX, nY, nFuente, nTexto, nTiempo, nFuncionTerminado) { 
...
//                    console.log(i, this.Enemigos[i]);
                    ...
                    this.Disparos.push(new this.Disparo(Math.floor(this.Enemigos[i].X + (this.TipoEnemigo[this.Enemigos[i].Tipo].Ancho / 2)), Math.floor(this.Enemigos[i].Y + (this.TipoEnemigo[this.Enemigos[i].Tipo].Alto)) , Angulo));
                }
            }
        }        
    };
    ...
    this.EjecutarEstado[4] = function() {
    
    };
    
    // Terminado
    this.EjecutarEstado[5] = function() {
    ...
    this.ActualizarAniDisparoTanque = function(Indice, Valor) {
    ...
    this.TerminarAniDisparoTanque = function() {  this.BloquearControles = false;  };
...
//        console.log(this.Mouse, this.Tanque.Angulo);
    ...
    this.FPS = function() {
        ...
        if (Tick > this.FPS_UltimoTick) {
            ...
            document.getElementById("SpaceInvaders404_Debug_FPS").innerHTML = this.FPS_Contador + " FPS";
            this.FPS_Contador = 0;
        }
        else {
            this.FPS_Contador ++;
