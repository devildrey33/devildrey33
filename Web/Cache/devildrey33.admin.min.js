...
$Base.TimerTiempoSesion=setTimeout(function(){$Admin.Desloguear();},30*60*1000);};$Base.ResetearTimerTiempoSesion();document.getElementById('devildrey33_Password').setAttribute('type','text');};this.Desloguear=function(){console.log("Admin.Desloguear");$Base.Cargando("TRUE");nAjax=$.post($Base.Raiz+"cmd/Desloguear.cmd");nAjax.done(function(data){if($Base.JSON_Parse(data)===false)return;console.log("Admin.Desloguear");$("#BarraNavegacion_Explorador").html(Datos["HTML"]);$Lab.EnlazarEventosExplorador();$Base.Cargando("FALSE");$("#ErroresPHP_Info").html(Datos["ErroresPHP"]);if(Datos["ErroresPHP"]!==""){$Base.MostrarErroresPHP();}
...
$Base.Cargando("TRUE");$Lab.Original=Codigo;$.post($Base.Raiz+"cmd/LabGuardarEjemplo.cmd",{"Archivo":Archivo,"Codigo":Codigo}).done(function(data){if($Base.JSON_Parse(data)===false)return;if(Datos["Estado"]===1){console.log("Admin.Lab_Guardar Ajax Error! no es admin");$("#BarraNavegacion_Explorador").html(Datos["HTML"]);$Lab.EnlazarEventosExplorador();$('body').attr({'administrador33':false});$Base.ClickMenu(0);setTimeout(function(){$('#Marco33').html('');},500);$Base.MostrarLogin($Admin.Lab_Guardar);}
...
$Admin.Log=new function(){this.IpAdmin="0.0.0.0";this.ArchivoChange=function(){var Combo=document.getElementById("Log_ArchivoActual");this.CargarArchivo(Combo.options[Combo.selectedIndex].innerHTML);};this.CargarArchivo=function(Archivo){$Base.Cargando("TRUE");$("#Log_Lista").html("").css({display:"none"});$("#Log_Stats").html("");$("#MarcoNavegacion").css({cursor:'progress'});$("#Log_Cargando").attr("visible",true);$("#Log_CargandoF1").css({color:"#fff"});nAjax=$.post($Base.Raiz+"cmd/ObtenerLog.cmd",{"Archivo":Archivo}).done(function(data){if($Base.JSON_Parse(data)===false)return;document.getElementById("Log_Datos").value=Datos["DatosLog"];$("#ErroresPHP_Info").html(Datos["ErroresPHP"]);if(Datos["ErroresPHP"]!==""){$Base.MostrarErroresPHP();}
