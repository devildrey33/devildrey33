...
$Base.Cargando("TRUE");$Lab.Original=Codigo;$.post($Base.Raiz+"cmd/LabGuardarEjemplo.cmd",{"Archivo":Archivo,"Codigo":Codigo}).done(function(data){Datos=$Base.JSON_Parse(data);if(Datos===false)return;if(Datos["Estado"]===1){console.log("Admin.Lab_Guardar Ajax Error! no es admin");$("#BarraNavegacion_Explorador").html(Datos["HTML"]);$Lab.EnlazarEventosExplorador();$('body').attr({'administrador33':false});$Base.ClickMenu(0);setTimeout(function(){$('#Marco33').html('');},500);$Base.MostrarLogin($Admin.Lab_Guardar);}
...
Codigo+="<tr>"+"<td style='color:green' title='"+this.ArrayDocumentos[i]["Url"]+"'>"+this.ArrayDocumentos[i]["Url"]+"</td>"+"<td>"+"<span class='Log_StatBarra' style='background-color:green; width:"+(300*this.ArrayDocumentos[i]["Contador"]/this.ArrayDocumentos[0]["Contador"])+"px'>"+this.ArrayDocumentos[i]["Contador"]+"</span></td>"+"</tr>";}
...
Codigo+="<tr>"+"<td style='color:purple' title='"+this.ArrayErrores[i]["Url"]+"'>"+this.ArrayErrores[i]["Url"]+"</td>"+"<td>"+"<span class='Log_StatBarra' style='background-color:purple; width:"+(300*this.ArrayErrores[i]["Contador"]/this.ArrayErrores[0]["Contador"])+"px'>"+this.ArrayErrores[i]["Contador"]+"</span></td>"+"</tr>";}
...
Codigo+="<tr>"+"<td style='color:grey' title='"+this.ArrayImagenes[i]["Url"]+"'>"+this.ArrayImagenes[i]["Url"]+"</td>"+"<td>"+"<span class='Log_StatBarra' style='background-color:grey; width:"+(300*this.ArrayImagenes[i]["Contador"]/this.ArrayImagenes[0]["Contador"])+"px'>"+this.ArrayImagenes[i]["Contador"]+"</span></td>"+"</tr>";}
...
Codigo+="<tr>"+"<td class='"+this.ColorRelevancia(this.ArrayObjetos[i]["Tipo"])+"' title='"+this.ArrayObjetos[i]["Url"]+"'>"+this.ArrayObjetos[i]["Url"]+"</td>"+"<td>"+"<span class='Log_StatBarra' style='width:"+(300*this.ArrayObjetos[i]["Contador"]/this.ArrayObjetos[0]["Contador"])+"px'>"+this.ArrayObjetos[i]["Contador"]+"</span></td>"+"</tr>";}
...
var Ret={"Url":Url,"Contador":1,"Tipo":Tipo};this.ArrayObjetos.push(Ret);};this.ExpandirIp=function(Id,MostrarTodo){var PosIp=Id.slice(10);if(this.Ips[PosIp]["Abierto"]===false){var Checks=this.ObtenerChecks();$("#"+Id).attr("abierto",true);var ColorNavegador="black";switch(this.Ips[PosIp]["Info"]["Tipo"]){case 0:ColorNavegador='Log_Cliente_Navegador';break;case 1:ColorNavegador='Log_Cliente_Bot';break;case 2:ColorNavegador='Log_Cliente_Desconocido';break;}
...
this.Ips[PosIp]["Abierto"]=!this.Ips[PosIp]["Abierto"];};this.TipoUrl=function(Url,Ret){var nUrl=Url.toLowerCase();if(nUrl.indexOf("banearip.php")!==-1){this.TotalTipos["Ataques"]++;return 9;}
...
this.TotalTipos["Otros"]++;return 0;};this.AgregarLinea=function(Linea,NumLinea){var Datos=Linea.split(" ");var TotalDatos=Datos.length;var Ret={Ip:"0.0.0.0",Datos:[{}],Ua:[""],Abierto:false,Destacados:[],Fecha:""};var Ua="";for(var i=12;i<Datos.length-1;i++){Ua+=Datos[i]+" ";}
...
return{"Nombre":Nombre,"NombreCorto":NombreCorto,"Version":Version,"Plataforma":Plataforma,"PlataformaCorto":PlataformaCorto,"Tipo":Tipo};};this.ClickCheckOpciones=function(){var Checks=this.ObtenerChecks();var Checks2={0:document.getElementById('ChNavegadores').checked,1:document.getElementById('ChBots').checked,2:document.getElementById('ChDesconocido').checked};for(var i=0;i<this.Ips.length;i++){var Pos=-1;if(Checks2[this.Ips[i]["Info"]["Tipo"]]===true){for(var e=0;e<this.Ips[i]["Destacados"].length;e++){if(Checks[this.Ips[i]["Destacados"][e]["Relevancia"]]===true){Pos=e;break;}}
if(Pos>-1){document.getElementById("EntradaLog"+i).style.display="table";var Url=$("#EntradaLog"+i+" .EntradaLog_Url");Url.attr('class',"EntradaLog_Url "+this.ColorRelevancia(this.Ips[i]["Destacados"][Pos]["Relevancia"]));Url.html(this.Ips[i]["Destacados"][Pos]["Url"]);if(this.Ips[i]["Abierto"]===true){this.Ips[i]["Abierto"]=false;this.ExpandirIp("EntradaLog"+i);}}
