...
this.Columnas[i]+=1;}}});
var Banner_AdiestramientoCubos3D=function(){ObjetoBanner.call(this,"THREE");this.TamCubo=0;this.Escena=new THREE.Scene();this.Camara=new THREE.PerspectiveCamera(40,this.Ancho/this.Alto,0.5,10000);this.Camara.position.set(0,0,2000);this.Escena.add(this.Camara);this.TipoAni=0;this.CharAni=0;var Divisiones=9;var Tam=1000;this.TamCubo=(Tam/(Divisiones/2));this.YBase=(this.TamCubo/2)+this.TamCubo;this.Parrilla=new THREE.GridHelper(Tam,Divisiones,new THREE.Color(0xcccccc),new THREE.Color(0x999999));this.Parrilla.position.y=this.TamCubo;this.GrupoCubos=new THREE.Object3D();this.GrupoCubos.add(this.Parrilla);t=this.TamCubo*0.7;var geometry=new THREE.BoxGeometry(t,t,t);for(var z=-(Divisiones/2);z<(Divisiones/2);z++){for(var x=-(Divisiones/2);x<(Divisiones/2);x++){var Grupo=new THREE.Object3D();var Cubo=new THREE.Mesh(geometry,new THREE.MeshBasicMaterial({color:"rgb(0,50,0)",}));Grupo.Ani=new this.AniColumna(1,1,0);Grupo.add(Cubo);Grupo.position.set((x*this.TamCubo)+(this.TamCubo/2),this.YBase,(z*this.TamCubo)+(this.TamCubo/2));this.CrearMarcoCubo(Grupo,t);this.GrupoCubos.add(Grupo);}}
this.RandAni();this.GrupoCubos.rotation.x=0.55;this.Escena.add(this.GrupoCubos);var pointLight=new THREE.PointLight(0xFF0000);pointLight.position.set(0,10,0);this.Escena.add(pointLight);this.Cargando(false);};Banner_AdiestramientoCubos3D.prototype=Object.assign(Object.create(ObjetoBanner.prototype),{constructor:Banner_AdiestramientoCubos3D,Nombre:"Adiestramiento de cubos",IdeaOriginal:"devildrey33",URL:"",NombreURL:"",RotacionCam:Rand(0.002,-0.002),AvanceCamZ:0.1,Redimensionar:function(){},Pintar:function(){var rnd=Rand();if(rnd<0.15){rnd=RandInt(4);if(rnd===0){if(this.RotacionCam>-0.004){this.RotacionCam-=0.0003;}}
...
var AniTerminada=true;for(var c=1;c<this.GrupoCubos.children.length;c++){var Cubo=this.GrupoCubos.children[c];if(Cubo.Ani.FaseAni===0){if(Cubo.Ani.Retardo<0){Cubo.Ani.Retardo+=16;}
...
var Banner_WormHole=function(){ObjetoBanner.call(this,"THREE");this.Escena=new THREE.Scene();window.scene=this.Escena;this.Camara=new THREE.PerspectiveCamera(75,this.Ancho/this.Alto,0.5,1000);this.Camara.position.set(0,(((this.Alto/2)-240)*0.20),500);this.Escena.add(this.Camara);this.H=0;this.H2=0.05;this.TexturasCargadas=0;this.TotalTexturas=2;var LoaderT1=new THREE.TextureLoader().load('/Web/Graficos/Banner_WormHole11.jpg',function(T){T.wrapT=T.wrapS=THREE.RepeatWrapping;T.repeat.set(1,2);$Banner.Tunel=new THREE.Mesh(new THREE.CylinderGeometry(50,50,1000,16,32,true),new THREE.MeshBasicMaterial({color:0x666666,transparent:true,alphaMap:T,side:THREE.BackSide,}));$Banner.Tunel.rotation.x=90*(Math.PI/180);$Banner.Tunel.position.z=128;$Banner.Escena.add($Banner.Tunel);$Banner.Textura=T;$Banner.TexturasCargadas++;if($Banner.TexturasCargadas===$Banner.TotalTexturas){$Banner.Cargando(false);}});var LoaderT2=new THREE.TextureLoader().load('/Web/Graficos/Banner_WormHole1.jpg',function(T){T.wrapT=T.wrapS=THREE.RepeatWrapping;T.repeat.set(1,2);$Banner.Tunel2=new THREE.Mesh(new THREE.CylinderGeometry(60,60,1000,16,32,true),new THREE.MeshBasicMaterial({color:0x666666,transparent:true,alphaMap:T,side:THREE.BackSide,}));$Banner.Tunel2.rotation.x=90*(Math.PI/180);$Banner.Tunel2.position.z=128;$Banner.Escena.add($Banner.Tunel2);$Banner.Textura2=T;$Banner.TexturasCargadas++;if($Banner.TexturasCargadas===$Banner.TotalTexturas){$Banner.Cargando(false);}});this.Camara.lookAt(this.Escena.position);};Banner_WormHole.prototype=Object.assign(Object.create(ObjetoBanner.prototype),{constructor:Banner_WormHole,Nombre:"WormHole",IdeaOriginal:"devildrey33",URL:"",NombreURL:"",Redimensionar:function(){},Scroll:function(){},MouseMove:function(X,Y){this.Camara.position.x=(((this.Ancho/2)-X)*0.040);this.Camara.position.y=(((this.Alto/2)-Y)*0.20);},Pintar:function(){if(typeof(this.Tunel)!=='undefined'){this.H+=0.001;if(this.H>1){this.H=0;}
this.Tunel.material.color.setHSL(this.H,1,0.7);this.Tunel.rotation.y+=0.02;this.Textura.offset.z+=0.01;this.Textura.offset.y-=0.015;}
if(typeof(this.Tunel2)!=='undefined'){this.H2+=0.001;if(this.H2>1){this.H2=0;}
this.Tunel2.material.color.setHSL(this.H2,1,0.7);this.Tunel2.rotation.y-=0.017;this.Textura2.offset.z+=0.005;this.Textura2.offset.y-=0.010;}
