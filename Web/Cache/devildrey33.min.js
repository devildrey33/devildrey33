...
var Banner_Lista=[Banner_Espacio2D,Banner_AdiestramientoCubos3D,Banner_WormHole,Banner_Sinusoidal,Banner_GeometriaBasica,Banner_EsferaVShader,Banner_ReunionDeCirculos,Banner_MultiCubo];var fPos=0;if(typeof(Pos==="undefined")){fPos=RandInt(Banner_Lista.length);}
else{fPos=Pos}
...
console.log("Base.IdentificarEntrada : "+Ret["TipoPagina"],Ret);return Ret;};this.BuscarEntrada=function(nURL){for(i=0;i<EntradasBlog.length;i++){if(nURL.indexOf(EntradasBlog[i]["URL"])!==-1){return i;}}
...
this.Cabecera.setAttribute("cargando",true);this.Cabecera.setAttribute("animar",true);this.Canvas=document.getElementById("Cabecera_Canvas");this.Ancho=0;this.Alto=0;if(this.OpcionesCanvas.Ancho!=="Auto"){this.Ancho=this.OpcionesCanvas.Ancho;}
...
else if(this.OpcionesCanvas.Tipo.toLowerCase()==='three'){if(this.PixelRatio()>1){this.Context=new THREE.WebGLRenderer({canvas:this.Canvas});console.log("ObjetoCanvas iniciado en modo THREE sin antialias");}
...
return true;};ObjetoCanvas.prototype.Terminar=function(){if(this.RAFID!==0){window.cancelAnimationFrame(this.RAFID);}
if(this.OpcionesCanvas.Entorno==="Normal"){this.Cabecera.removeEventListener('mousedown',this.hEventoMousePresionado);this.Cabecera.removeEventListener('mouseup',this.hEventoMouseSoltado);window.removeEventListener('keydown',this.hEventoTeclaPresionada);window.removeEventListener('keyup',this.hEventoTeclaSoltada);}
...
else{this.FPS_Contador++;}};var BufferCanvas=function(Ancho,Alto){this.Canvas=document.createElement("canvas");this.Canvas.setAttribute("width",Ancho);this.Canvas.setAttribute("height",Alto);this.Context=this.Canvas.getContext("2d");this.Ancho=Ancho;this.Alto=Alto;};
var FuncionesTiempo={Linear:function(Tiempo){return Tiempo;},SinIn:function(Tiempo){return 1-Math.cos(Tiempo*Math.PI/2);},SinOut:function(Tiempo){return Math.sin(Tiempo*Math.PI/2);},SinInOut:function(Tiempo){return-0.5*(Math.cos(Math.PI*Tiempo)-1);}};var ObjetoAnimacion=function(){this.Animaciones=[];this.EnPausa=false;this.Actualizar=function(nTick){if(this.EnPausa===true){return;}
var Tick;if(typeof(nTick)==='undefined'){Tick=Date.now();}
else{Tick=nTick;}
for(var i=this.Animaciones.length-1;i>-1;i--){if(this.Animaciones[i].Actualizar(Tick)===true){this.Animaciones.splice(i,1);}}};this.Pausa=function(){this.EnPausa=true;};this.Reanudar=function(){var Tick=Date.now();for(var i=this.Animaciones.length-1;i>-1;i--){this.Animaciones[i]._UltimoTick=Tick-1;}
this.EnPausa=false;};this.Limpiar=function(){this.Animaciones=[];};this.Crear=function(Pasos,Opciones){var Ani=new this.Animacion(Pasos,Opciones);this.Animaciones.push(Ani);return Ani;};this.Animacion=function(ArrayPasos,Opciones){this._UltimoTick=0;this._Pasos=ArrayPasos;this._PosPasos=1;this._PasoOrig=this._Pasos[this._PosPasos-1];this._PasoDest=this._Pasos[this._PosPasos];this._Avance=0;this._Opciones={Repetir:0,FuncionActualizar:function(Valores){},FuncionTerminado:function(){}};if(typeof(Opciones)!=='undefined'){if(typeof Opciones.Repetir!=="undefined"){this._Opciones.Repetir=Opciones.Repetir;}
...
this.Terminado=false;for(var Paso in this._Pasos){if(typeof this._Pasos[Paso].Tiempo==="undefined"){this._Pasos[Paso].Tiempo=1;}
...
this._Opciones.FuncionActualizar(this);this.Cancelar=function(){console.log("ObjetoAnimacion.Cancelar");this._Opciones.FuncionTerminado();this.Terminado=true;};this.Terminar=function(){console.log("ObjetoAnimacion.Terminar");this._Opciones.FuncionTerminado();this.Terminado=true;for(var Indice in this._Pasos[this._Pasos.length-1].Paso){this[Indice]=this._Pasos[this._Pasos.length-1].Paso[Indice];}
this._Opciones.FuncionActualizar(this);};this.Actualizar=function(t){if(this.Terminado===true){return true;}
if(this._UltimoTick!==0){var TiempoFrame=t-this._UltimoTick;if(this._PasoDest.Retraso>0){this._PasoDest.Retraso=this._PasoDest.Retraso-TiempoFrame;}
...
this._Opciones.FuncionActualizar(this);this._PosPasos++;if(this._PosPasos<this._Pasos.length){this._PasoOrig=this._Pasos[this._PosPasos-1];this._PasoDest=this._Pasos[this._PosPasos];this._Avance=0;}
else{if(this._Opciones.Repetir===0){this._Opciones.FuncionTerminado();this.Terminado=true;}
else{this._Opciones.Repetir--;this._PosPasos=1;this._PasoOrig=this._Pasos[this._PosPasos-1];this._PasoDest=this._Pasos[this._PosPasos];this._Avance=0;}}}}}
this._UltimoTick=t;return this.Terminado;};};};
...
console.log(Alto);this.ControlContenedor.style.height=Alto+"px";var Diferencia=Alto-AlturaInicial;var Nodo=this.ControlContenedor.parentNode;while(Nodo.className!=="ObjetoTest"){if(Nodo.className==="ObjetoTest_Contenedor"){Nodo.style.height=(parseInt(Nodo.style.height)+Diferencia)+"px";}
