...
else{$Base.MostrarMensaje("Only you can vote one time per page.");}}};this.MostrarMensaje=function(Mensaje,Alerta){console.log("Base.MostrarMensaje("+Mensaje+")");$("#VentanaMensaje > p").html(Mensaje);$("#VentanaMensaje > div > button").off("click.button").on("click.button",function(){$("#VentanaMensaje").attr({"visible":"false"});});if(typeof(Alerta)==="undefined")Alerta="false";$("#VentanaMensaje").attr({"visible":"true","Alerta":Alerta});setTimeout(function(){$("#VentanaMensaje > div > button").focus();},100);};this.MostrarErrorAjax=function(Error,VolverIndice,Excepcion){console.log("Base.MostrarErrorAjax("+Error+", "+VolverIndice+")");this.LogoCargando("FALSE");this.Cargando("FALSE");switch(Error){case 403:$("#VentanaError > p").html("Error 403! no se permite el acceso al archivo solicitado.");break;case 404:$("#VentanaError > p").html("Error 404! el archivo solicitado no existe.");break;default:$("#VentanaError > p").html("Error 500! Es posible que el servidor no esté disponible en estos momentos. ("+Excepcion+")");break;}
if(VolverIndice===true){this.Entrada["TipoPagina"]="Error404";$("#VentanaError > p").html($("#VentanaError > p").html()+" Pulsa aceptar para ser re-dirigido al indice.");$("#VentanaError > div > button").off("click.VentanaError").on("click.VentanaError",function(){$("#VentanaError").attr({"visible":"false"});$Base.CargarURL("/");});}
...
$("#VentanaError").attr({"visible":"true"});setTimeout(function(){$("#VentanaError > div > button").focus();},100);this.ActualizarBarraNavegacion();};this.IdentificarEntrada=function(URL,URLReal){$("#BarraNavegacion_Encuesta").removeAttr("Visible");var Ret={"Titulo":"devildrey33","URL":URL,"URLReal":URLReal,"Imagen":"","TipoPagina":"No identificado","Pos":-2,"Idioma":"es"};if(URLReal.indexOf("/Categorias/")>-1||URL==="/"){Ret["TipoPagina"]="Indice";$("body").attr({"Tipo":Ret["TipoPagina"]});console.log("Base.IdentificarEntrada : "+Ret["TipoPagina"],Ret);return Ret;}
