...
$("#VentanaError").attr({"visible":"true"});setTimeout(function(){$("#VentanaError > div > button").focus();},100);this.ActualizarBarraNavegacion();};this.IdentificarEntrada=function(URL,URLReal){var Ret={"Titulo":"devildrey33","URL":URL,"URLReal":URLReal,"Imagen":"","TipoPagina":"Desconocido","Pos":-2,"Idioma":"es"};if(URLReal.indexOf("Categorias/")>-1||URL==="/"+this.RaizRelativa||URL===this.Raiz){Ret["TipoPagina"]="Indice";$("body").attr({"Tipo":Ret["TipoPagina"]});console.log("Base.IdentificarEntrada : "+Ret["TipoPagina"],Ret);return Ret;}
...
console.log("Base.Loguear("+pass+")");this.Cargando("TRUE");$.post($Base.Raiz+"cmd/Loguear.cmd",{"l":l,"p":pass}).done(function(data){Datos=JSON.parse(data);if(Datos.Mensaje==="Correcto!"){$Base.CargarJS("ObjetoAdmin.js",function(){$Admin.Iniciar();});localStorage["Comentarios_Usuario"]=$("#devildrey33_Usuario").val();$("#devildrey33_Password").val("");$("#VentanaLogin").attr({visible:false});$("#Marco33").html(Datos.HTMLAdmin);$("#BarraNavegacion_Explorador").html(Datos.ExplorarLab);$Lab.EnlazarEventosExplorador();}
...
else if($(window).scrollTop()<190&&typeof Header.attr("animar")==="undefined"){console.log("Base.AnimarCabeceraBlog [animar=true]",Header);Header.attr({"animar":"true"});}}};this.ComprobarScrollVotacion=function(){console.log("Base.ComprobarScrollVotacion()",$("#Comentarios_Datos").offset())
