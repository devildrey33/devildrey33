...
if(URL.indexOf("/Web/")>-1||URL.indexOf("/FaqBarba")>-1){Ret["TipoPagina"]="Web";nURL="";}
...
console.log("Base.Loguear("+pass+")");this.Cargando("TRUE");$.post("/cmd/Loguear.cmd",{"l":l,"p":pass}).done(function(data){Datos=JSON.parse(data);if(Datos.Mensaje==="Correcto!"){$Base.CargarJS("ObjetoAdmin.js",function(){$Admin.Iniciar();});localStorage["Comentarios_Usuario"]=$("#devildrey33_Usuario").val();$("#devildrey33_Password").val("");$("#VentanaLogin").attr({visible:false});$("#Marco33").html(Datos.HTMLAdmin);$("#BarraNavegacion_Explorador").html(Datos.ExplorarLab);$("#BarraNavegacion_Explorador .Lab_Archivo").off("click").on("click",function(){$Lab.ClickArchivo($(this));});$("#BarraNavegacion_Explorador .Lab_Directorio").off("click").on("click",function(){$Lab.ClickDirectorio($(this));});}
...
$.getScript("/Web/JS/"+Nombre,function(data,textStatus,jqxhr){console.log("Base.CargarJS("+Nombre+")");$Base.FuncionCargarJS();}).fail(function(jqxhr,settings,exception){console.log("Base.CargarJS Error Ajax",jqxhr,settings,exception);$Base.MostrarErrorAjax(jqXHR.status,false);});};this.CargarCSS=function(Nombre){for(var i=0;i<this.CSSDinamico.length;i++){if(this.CSSDinamico[i]===Nombre){console.log("Base.CargarCSS("+Nombre+") ya se había cargado...");return;}}
...
else if($(window).scrollTop()<190&&typeof Header.attr("animar")==="undefined"){console.log("Base.AnimarCabeceraBlog [animar=true]",Header);Header.attr({"animar":"true"});}}};this.ComprobarScrollVotacion=function(){if($(window).scrollTop()>($(document).height()-($(window).height()*2))){console.log("Base.ComprobarScrollVotacion");if(typeof localStorage["Voto_"+Pagina]==="undefined"){$("#BarraNavegacion_Votacion").attr({"Mostrar":true});$("#BarraNavegacion_Votacion .VotarDocumento_Estrellas > button").off("click").on("click",function(e){$Base.VotarWeb($(this).html());});}}};};Date.prototype.toDateInputValue=(function(){var local=new Date(this);local.setMinutes(this.getMinutes()-this.getTimezoneOffset());return local.toJSON().slice(0,10);});window.addEventListener('popstate',function(event){$Base.CALLBACK_Histroial(event);},false);$(window).load(function(){$Base.Iniciar();});$(window).scroll(function(){$Base.AnimarCabeceraBlog();$Base.ComprobarScrollVotacion();});
$Indice=new function(){this.Cargando=false;this.Iniciar=function(){console.log("Indice.Iniciar");$("#Categorias").off("click").on("click",function(e){$Indice.ClickCategorias();});$("#Categorias > div > div").off("click").on("click",function(e){$Base.CargarURL($(this).attr("path"));});};this.ClickCategorias=function(){Cat=$("#Categorias");console.log("Indice.ClickCategorias",$("#Categorias > div").height(),Cat.height());if(Cat.height()==19){Cat.height(40+$("#Categorias > div").height());}
else{Cat.height(19);}};this.ComprobarScroll=function(){console.log("Indice.ComprobarScroll",$("#MarcoIndice > div[finscrollinfinito]").length);if($("#MarcoIndice > div[finscrollinfinito]").length===0&&this.Cargando===false&&$("#MarcoIndice").length!==0){if($(window).scrollTop()>($(document).height()-($(window).height()*2))){this.Cargando=true;$Base.Cargando("TRUE");Inicio=$("#MarcoIndice > .Articulo_Portada").length;nAjax=$.post("/cmd/IndiceObtener10Mas.cmd",{"Categoria":$("#Categorias").attr("categoria"),"Inicio":Inicio});nAjax.done(function(data){$("div[puntoscroll=true]").removeAttr("puntoscroll");$("#MarcoIndice").html($("#MarcoIndice").html()+data);$Base.Cargando("FALSE");$Indice.Cargando=false;});nAjax.fail(function(jqXHR,textStatus,tError){console.log("Indice.ComprobarScroll Error ajax",jqXHR,textStatus,tError);$Indice.Cargando=false;$Base.MostrarMensaje("Error cargando los 10 siguientes articulos");$Base.Cargando("FALSE");});}}};};$(window).scroll(function(){$Indice.ComprobarScroll();});
...
else{this.CargarArchivo(Ret);}};this.CargarArchivo=function(Archivo,ID){$Base.Cargando("TRUE");var nopush=false;console.log("Lab.CargarArchivo",Archivo);if(Archivo===null){Archivo=this.Archivo;nopush=true;}
