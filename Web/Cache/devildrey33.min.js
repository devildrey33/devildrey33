...
var Banner_Lista=[Banner_Espacio2D,Banner_AdiestramientoCubos3D,Banner_WormHole,Banner_Anillos,Banner_Sinusoidal,Banner_GeometriaBasica,Banner_EsferaVShader];var fPos=0;if(typeof(Pos==="undefined")){fPos=RandInt(Banner_Lista.length);}
else{fPos=Pos}
...
if(cURL.indexOf("web/")>-1){Ret["TipoPagina"]="Web";nURL="";}
if(cURL.indexOf("sobre-el-autor")>-1||cURL.indexOf("faqbarba")>-1){Ret["TipoPagina"]="Simple";nURL="";}
...
try{if(this.OpcionesCanvas.Tipo.toLowerCase()==='2d'){this.Context=this.Canvas.getContext("2d");}
else if(this.OpcionesCanvas.Tipo.toLowerCase()==='three'){if(this.PixelRatio()>1){this.Context=new THREE.WebGLRenderer({canvas:this.Canvas});}
...
var Banner_Anillos=function(){if(ObjetoCanvas.call(this,{'Tipo':'2d','Ancho':'Auto','Alto':'Auto','Entorno':'Banner','MostrarFPS':true,'ElementoRaiz':document.body})===false){return false;}
this.Iniciar();this.Cargando(false);};Banner_Anillos.prototype=Object.assign(Object.create(ObjetoCanvas.prototype),{constructor:Banner_Anillos,Nombre:"Anillos",IdeaOriginal:"devildrey33",URL:"/Lab/Ejemplos/BannerTest/Anillos.html",NombreURL:"Lab : Anillos",Invertir:false,Circulo:function(Padre){this.Padre=Padre;var CentroX=(this.Padre.Ancho/2);this.CentroY=(this.Padre.Alto/2);this.Angulo=Rand(360);this.Distancia=15+Rand(CentroX+100);this.Tam=Rand(4,1);this.Avance=(Rand(0.008,0.0005)/4)*this.Tam;this.X=CentroX+(this.Distancia*Math.cos(this.Angulo));this.Y=(this.Padre.Alto/2)+(this.Distancia*Math.sin(this.Angulo));this.Color="rgb("+RandInt(255,20)+","+RandInt(255,20)+","+RandInt(255,20)+")";this.Avanzar=function(){if(Padre.Invertir===false){this.Angulo+=this.Avance;}
else{this.Angulo-=this.Avance*1.5;}
this.X=(this.Padre.Ancho/2)+(this.Distancia*Math.cos(this.Angulo));this.Y=(this.Padre.Alto/2)+(this.Distancia*Math.sin(this.Angulo));};},Iniciar:function(){this.TotalCirculos=350;this.Circulos=[];for(var i=0;i<this.TotalCirculos;i++){this.Circulos[i]=new this.Circulo(this);}
this.Context.fillStyle='rgb(49, 46, 53)';this.Context.fillRect(0,0,this.Ancho,this.Alto);},MouseEnter:function(event){this.Invertir=true;},MouseLeave:function(event){this.Invertir=false;},Pintar:function(){this.Context.fillStyle=(this.Invertir===true)?'rgba(49, 46, 53, 0.02)':'rgba(49, 46, 53, 0.09)';this.Context.fillRect(0,0,this.Ancho,this.Alto);for(var i=0;i<this.TotalCirculos;i++){this.Circulos[i].Avanzar();this.Context.beginPath();this.Context.fillStyle=this.Circulos[i].Color;this.Context.arc(this.Circulos[i].X,this.Circulos[i].Y,this.Circulos[i].Tam,0,2*Math.PI,false);this.Context.fill();}}});
...
this.Iniciar();this.Cargando(false);return true;};SobreDevildrey33.prototype=Object.assign(Object.create(ObjetoCanvas.prototype),{constructor:SobreDevildrey33,Redimensionar:function(){},Scroll:function(){},MouseMove:function(Evento){this.Mouse.x=(Evento.clientX/this.Ancho)*2-1;this.Mouse.y=(Evento.clientY/this.Alto)*2-1;},MousePresionado:function(Evento){},MouseSoltado:function(Evento){},MouseEnter:function(Evento){},MouseLeave:function(Evento){},TeclaPresionada:function(Evento){},TeclaSoltada:function(Evento){},Iniciar:function(){this.Context.shadowMap.enabled=true;this.Context.shadowMap.type=THREE.PCFSoftShadowMap;this.RayCaster=new THREE.Raycaster();this.Mouse=new THREE.Vector2();this.Escena=new THREE.Scene();this.Camara=new THREE.PerspectiveCamera(75,this.Ancho/this.Alto,0.5,1000);this.Camara.position.set(0,0,1000);this.Escena.add(this.Camara);var MaterialSuelo=new THREE.MeshPhongMaterial({color:0x666666,shininess:200,specular:0x33AA33,shading:THREE.SmoothShading,});var MaterialPared=new THREE.MeshPhongMaterial({color:0x8888AA,shininess:200,specular:0x33AA33,shading:THREE.SmoothShading,});var MaterialCubo1=new THREE.MeshPhongMaterial({color:0x0000AA,shininess:300,specular:0x33AA33,shading:THREE.SmoothShading,transparent:true,opacity:0.7});var MaterialCubo2=new THREE.MeshPhongMaterial({color:0x0000AA,shininess:300,specular:0x33AA33,shading:THREE.SmoothShading,transparent:true,opacity:0.7});hemiLight=new THREE.HemisphereLight(0xddeeff,0x0f0e0d,0.02);hemiLight.position.set(0,50,-10);hemiLight.castShadow=true;this.Escena.add(hemiLight);hemiLight.intensity=0.7;this.Suelo=new THREE.Mesh(new THREE.BoxGeometry(300,1,300),MaterialSuelo);this.Suelo.castShadow=false;this.Suelo.receiveShadow=true;this.Escena.add(this.Suelo);this.ParedI=new THREE.Mesh(new THREE.BoxGeometry(1,150,300),MaterialPared);this.ParedI.position.x=-150;this.ParedI.position.y=75;this.ParedI.castShadow=false;this.ParedI.receiveShadow=true;this.Escena.add(this.ParedI);this.ParedD=new THREE.Mesh(new THREE.BoxGeometry(1,150,300),MaterialPared);this.ParedD.position.x=150;this.ParedD.position.y=75;this.ParedD.castShadow=false;this.ParedD.receiveShadow=true;this.Escena.add(this.ParedD);this.ParedC=new THREE.Mesh(new THREE.BoxGeometry(300,150,1),MaterialPared);this.ParedC.position.z=-150;this.ParedC.position.y=75;this.ParedC.castShadow=false;this.ParedC.receiveShadow=true;this.Escena.add(this.ParedC);this.Cubo=new THREE.Mesh(new THREE.BoxGeometry(12,12,12),MaterialCubo1);this.Cubo.castShadow=true;this.Cubo.receiveShadow=false;this.Cubo.position.set(-12,20,20);this.Escena.add(this.Cubo);this.Cubo2=new THREE.Mesh(new THREE.BoxGeometry(12,12,12),MaterialCubo1);this.Cubo2.castShadow=true;this.Cubo2.receiveShadow=false;this.Cubo2.position.set(12,20,20);this.Escena.add(this.Cubo2);this.PointLight=new THREE.PointLight(0xffee88,25000,200,10);this.PointLight.position.set(0,120,70);this.PointLight.castShadow=true;this.Escena.add(this.PointLight);this.AniAvanceCamara=ObjetoAnimacion.Crear([{'Paso':{z:500,y:150}},{'Paso':{z:70,y:35},'Tiempo':2250,'FuncionTiempo':FuncionesTiempo.SinInOut}],{"Repetir":0,FuncionActualizar:function(Indice,Valor){this.Camara.position[Indice]=Valor;}.bind(this)});this.AniCuboY1=ObjetoAnimacion.Crear([{'Paso':{y:35}},{'Paso':{y:40},Tiempo:750,FuncionTiempo:FuncionesTiempo.SinOut},{'Paso':{y:35},Tiempo:750,FuncionTiempo:FuncionesTiempo.SinIn},],{"Repetir":-1,FuncionActualizar:function(Indice,Valor){this.Cubo.position[Indice]=Valor;}.bind(this)});this.AniCuboY2=ObjetoAnimacion.Crear([{'Paso':{y:35}},{'Paso':{y:40},Tiempo:750,FuncionTiempo:FuncionesTiempo.SinOut,Retraso:375},{'Paso':{y:35},Tiempo:750,FuncionTiempo:FuncionesTiempo.SinIn},],{"Repetir":-1,FuncionActualizar:function(Indice,Valor){this.Cubo2.position[Indice]=Valor;}.bind(this)});this.AniPosLuz=ObjetoAnimacion.Crear([{'Paso':{x:-20,y:120,z:70}},{'Paso':{x:20,y:110,z:60},Tiempo:12000,FuncionTiempo:FuncionesTiempo.SinInOut},{'Paso':{x:-20,y:120,z:70},Tiempo:12000,FuncionTiempo:FuncionesTiempo.SinInOut},],{Repetir:-1,FuncionActualizar:function(Indice,Valor){this.PointLight.position[Indice]=Valor;}.bind(this)});this.Tiempo=Date.now();this.Avance=0;},Pintar:function(){ObjetoAnimacion.Actualizar(this.Tick);this.RayCaster.setFromCamera(this.Mouse,this.Camara);var intersects=this.RayCaster.intersectObjects(this.Escena.children);if(intersects[0]){if(intersects[0].object===this.Cubo){console.log("cubo");}
