...
var Banner_Lista=[Banner_ResplandorCircular,Banner_Colisiones,Banner_TranstornoLineal,Banner_Espacio2D,Banner_MatrixLluviaHexadecimal,Banner_Cubos3D];var fPos=0;if(typeof(Pos==="undefined")){fPos=RandInt(Banner_Lista.length);}
else{fPos=Pos}
...
var $Banner=null;ObjetoBanner=function(Tipo){$("#Cabecera_Canvas").remove();$(".Cabecera").append("<canvas id='Cabecera_Canvas'></canvas>");this.Canvas=document.getElementById("Cabecera_Canvas");if(typeof(Tipo)==="undefined"){this.Tipo="2d";}
else{this.Tipo=Tipo;}
console.log("constructor ObjetoBanner",this.Tipo);if(this.Tipo==="2d"){this.Context=this.Canvas.getContext("2d");}else if(this.Tipo==="THREE"){this.Context=new THREE.WebGLRenderer({canvas:this.Canvas,antialias:true});}
this.Ancho=0;this.Alto=0;this.RAFID=0;this.FPS_UltimoTick=Date.now()+1000;this.FPS_Contador=0;this.PIx2=Math.PI*2;this.FocoWeb=true;this.Depurar=false;$("#CabeceraAutorAni_HTML").html("<div>"+this.Nombre+"</div>"+"<div><span style='color:#AAA;'>Concepto original : </span><b>"+this.IdeaOriginal+"</b></div>"+"<a href='"+this.URL+"' target='_blank'>"+this.NombreURL+"</a>");$("#Cabecera_AutorAni > .BotonVentana:nth-child(1)").off("click").on("click",function(){$Base.Banner(-1);});$("#Cabecera_AutorAni > .BotonVentana:nth-child(3)").off("click").on("click",function(){$Base.Banner(-2);});this.EventoRedimensionar();this.RAFID=window.requestAnimationFrame(this.Actualizar);if(typeof(this.Escena)!=="undefined"){window.scene=this.Escena;}};ObjetoBanner.prototype.Actualizar=function(){if($Banner!==null){$Banner.FPS();$Banner.RAFID=window.requestAnimationFrame($Banner.Actualizar);$Banner.Pintar.apply($Banner);}};ObjetoBanner.prototype.EventoRedimensionar=function(){this.Ancho=document.getElementById("MarcoNavegacion").offsetWidth-60;this.Alto=this.Canvas.offsetHeight;this.Canvas.setAttribute("width",this.Ancho);this.Canvas.setAttribute("height",this.Alto);if(this.Tipo==="THREE"){this.Context.setSize(this.Ancho,this.Alto);if(this.Camara!==null&&typeof(this.Camara)!=='undefined'){this.Camara.aspect=this.Ancho/this.Alto;this.Camara.updateProjectionMatrix();}}
if(typeof(this.Redimensionar)!=="undefined"){this.Redimensionar.apply(this);}};ObjetoBanner.prototype.Pausa=function(){if(this.RAFID!==0&&this.Depurar===false){$(".Cabecera").attr({"animar":"false"});console.log("Banner.Pausa");window.cancelAnimationFrame(this.RAFID);this.RAFID=0;}};ObjetoBanner.prototype.Reanudar=function(){if(this.RAFID===0&&this.FocoWeb===true){$(".Cabecera").attr({"animar":"true"});this.RAFID=window.requestAnimationFrame(this.Actualizar);console.log("Banner.Reanudar RAFID = "+this.RAFID);}};ObjetoBanner.prototype.EventoScroll=function(){var Header=$(".Cabecera");if(Header.length>0){var PS=$(window).scrollTop();var Altura=Header.get(0).offsetHeight;if(PS>Altura){if($Banner!==null){$Banner.Pausa();}}
