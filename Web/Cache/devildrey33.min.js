...
this.Columnas[i]+=1;}}});
var Banner_AdiestramientoCubos3D=function(){ObjetoBanner.call(this,"THREE");this.TamCubo=0;this.Escena=new THREE.Scene();this.Camara=new THREE.PerspectiveCamera(45,this.Ancho/this.Alto,0.5,10000);this.Camara.position.set(0,2000,2000);this.Escena.add(this.Camara);this.TipoAni=0;this.CharAni=0;var Divisiones=7;var Tam=1000;this.TamCubo=(Tam/(Divisiones/2));this.YBase=(this.TamCubo/2)+this.TamCubo;this.Parrilla=new THREE.GridHelper(Tam,Divisiones,new THREE.Color(0xcccccc),new THREE.Color(0x999999));this.Parrilla.position.y=this.TamCubo;this.GrupoCubos=new THREE.Object3D();this.GrupoCubos.add(this.Parrilla);t=this.TamCubo*0.7;var geometry=new THREE.BoxGeometry(t,t,t);for(var z=-(Divisiones/2);z<(Divisiones/2);z++){for(var x=-(Divisiones/2);x<(Divisiones/2);x++){var Grupo=new THREE.Object3D();var Cubo=new THREE.Mesh(geometry,new THREE.MeshBasicMaterial({color:"rgb(0,50,0)",}));Grupo.Ani=new this.AniColumna(1,1,0);Grupo.add(Cubo);Grupo.position.set((x*this.TamCubo)+(this.TamCubo/2),this.YBase,(z*this.TamCubo)+(this.TamCubo/2));this.CrearMarcoCubo(Grupo,t);this.GrupoCubos.add(Grupo);}}
this.RandAni();this.Escena.add(this.GrupoCubos);var pointLight=new THREE.PointLight(0xFF0000);pointLight.position.set(0,10,0);this.Escena.add(pointLight);this.Cargando(false);};Banner_AdiestramientoCubos3D.prototype=Object.assign(Object.create(ObjetoBanner.prototype),{constructor:Banner_AdiestramientoCubos3D,Nombre:"Adiestramiento de cubos",IdeaOriginal:"devildrey33",URL:"/Lab/Ejemplos/BannerTest/AdiestramientoCubos.html",NombreURL:"Lab : Adiestramiento de cubos",RotacionCam:0,AvanceCamZ:0.1,VarCam:{VelX:0.0001,MinVelX:-0.005,MaxVelX:0.005,AniX:true},Redimensionar:function(){},AniCamara:function(){if(Rand()<0.01){switch(RandInt(2)){case 0:this.VarCam.AniX=true;break;case 1:this.VarCam.AniX=false;break;}}
if(this.VarCam.AniX===true){if(this.VarCam.VelX<this.VarCam.MaxVelX){this.VarCam.VelX+=0.0001;}}
else{if(this.VarCam.VelX>this.VarCam.MinVelX){this.VarCam.VelX-=0.0001;}}
var x=this.Camara.position.x;var z=this.Camara.position.z;this.Camara.position.x=x*Math.cos(this.VarCam.VelX)-z*Math.sin(this.VarCam.VelX);this.Camara.position.z=z*Math.cos(this.VarCam.VelX)+x*Math.sin(this.VarCam.VelX);this.Camara.lookAt(this.GrupoCubos.position);},Pintar:function(){this.AniCamara();var AniTerminada=true;for(var c=1;c<this.GrupoCubos.children.length;c++){var Cubo=this.GrupoCubos.children[c];if(Cubo.Ani.FaseAni===0){if(Cubo.Ani.Retardo<0){Cubo.Ani.Retardo+=16;}
