...
return;}
console.log("Base.Buscar "+$("#BarraPrincipal_MarcoBuscar_Edit").val());this.Cargando("TRUE");$.post("/cmd/Buscar",{"Palabras":$("#BarraPrincipal_MarcoBuscar_Edit").val()}).done(function(data){Datos=JSON.parse(data);$("#BarraPrincipal_MarcoBuscar_Resultado").html(Datos["HTML"]);$("#ErroresPHP_Info").html(Datos["ErrorPHP"]);$Base.Cargando("FALSE");$("#BarraPrincipal_MarcoBuscar a[href]:not([target])").off("click").on("click",function(e){e.preventDefault();e.stopImmediatePropagation();$Base.CargarURL($(this).attr("href"));$Base.ClickMenu(0);return false;});}).fail(function(jqXHR,textStatus,tError){console.log("Base.Buscar Error ajax",jqXHR,textStatus,tError);$Base.MostrarErrorAjax(jqXHR.status,false,tError);});};this.CargarURL=function(URL){$Lab.Modificado();$("#BarraNavegacion > .Menu > input[type=checkbox]").removeAttr("checked");this.LogoCargando("TRUE");if(URL.indexOf("/")===-1){Path=window.location.href.split('/');Path.pop();Path=Path.join('/');URL=Path+"/"+URL;nURL=Path+"/"+URL;}
