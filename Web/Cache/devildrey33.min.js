...
var Banner_Lista=[Banner_Espacio2D,Banner_MatrixLluviaHexadecimal,Banner_AdiestramientoCubos3D,Banner_WormHole,Banner_Sinusoidal,Banner_GeometriaBasica,Banner_ReunionDeCirculos,Banner_HexTunnel,Banner_MultiCubo,Banner_EsferaVShader];var fPos=0;if(typeof(Pos==="undefined")){fPos=RandInt(Banner_Lista.length-1);}
else{fPos=Pos}
...
return;}
console.log("Base.Buscar "+$("#BarraPrincipal_MarcoBuscar_Edit").val());this.Cargando("TRUE");$.post(this.Raiz+"cmd/Buscar",{"Palabras":$("#BarraPrincipal_MarcoBuscar_Edit").val()}).done(function(data){if($Base.JSON_Parse(data)===false)return;$("#BarraPrincipal_MarcoBuscar_Resultado").html(Datos["HTML"]);$("#ErroresPHP_Info").html(Datos["ErroresPHP"]);if(Datos["ErroresPHP"]!==""){$Base.MostrarErroresPHP();}
...
console.log("Base.CargarURL "+nURL);this.cURL=cURL;this.nURL=nURL;this.PeticionAjax=$.post(nURL,{"Regenerar":"todo","Categoria":"Todo","SinPlantilla":"true"}).done(function(data){if($Base.JSON_Parse(data)===false)return;if(Datos["HTML"].indexOf("<script>$Base.MostrarErrorAjax(404, false);</script>")===0){$Base.MostrarErrorAjax(404,false,'No se ha encontrado');}
...
console.log("Base.Loguear("+pass+")");this.Cargando("TRUE");$.post($Base.Raiz+"cmd/Loguear.cmd",{"l":l,"p":pass}).done(function(data,textStatus,jqXHR){if($Base.JSON_Parse(data)===false)return;if(Datos.Estado===0){localStorage["Comentarios_Usuario"]=$("#devildrey33_Usuario").val();$("#VentanaLogin").attr({visible:false});$("#Marco33").html(Datos.HTMLAdmin);$Base.CargarJS("devildrey33.admin.min.js",function(){$Admin.Iniciar();},true);$("#BarraNavegacion_Explorador").html(Datos.ExplorarLab);$Lab.EnlazarEventosExplorador();$Base.FPL();}
...
if(cURL==="/")nURL="/index.php";else nURL=cURL;this.PeticionAjax=$.post(nURL,{"Regenerar":"todo","Categoria":"Todo","SinPlantilla":"true"}).done(function(data){if($Base.JSON_Parse(data)===false)return;if(Datos["HTML"].indexOf("<script>$Base.MostrarErrorAjax(404, false);</script>")===0){$Base.MostrarErrorAjax(404,"false");}
...
this.PeticionAjax=$.post($Base.Raiz+"cmd/LabAbrirEjemplo.cmd",{"Archivo":Archivo,"ID":ID},function(data){if($Base.JSON_Parse(data)===false)return;if(Datos["Ret"]===404){$Base.MostrarErrorAjax(404,false);}
