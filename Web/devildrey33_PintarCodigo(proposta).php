...
/* TODO : el CSS necesita ser revisado, de momento tiene 3 estados : CSS, CSS_Reglas, CSS_Selector.
            -Hay que incluir estados CSS_Propiedad, y CSS_Valor para que no tome los colores hex como un selector y los pinte de verde
            -Se tienen que enumerar testear todas las posibles reglas : @keyframes, @font-face, @page, @media, @namespace, @supports, @charset, @document, @import
 *  */
    ...
    protected function _EscanearLinea($Linea, $Diccionario, $Delimitadores, &$Palabras, &$TotalPalabras, &$l, &$Estado, $Tipo, $AbrirDivLinea, $CerrarDivLinea, &$NuevoParsing, &$NuevoParsingLinea) {        
        ...
        switch ($Estado) {
            ...
            case "CSS_Selector" :
                ...
                break;
            case "CSS_Propiedad" :
                $Palabras[$TotalPalabras ++] = $this->_DivInicial($l, $this->_Color($Diccionario, 6), $Estado, $AbrirDivLinea, $Spans);
                break;
            case "CSS_Valor" :
                $Palabras[$TotalPalabras ++] = $this->_DivInicial($l, $this->_Color($Diccionario, 7), $Estado, $AbrirDivLinea, $Spans);
                break; 
        ...
        for ($i = 0; $i < strlen($Linea); $i++) { 
            switch ($Estado) {
                case "HTML" : case "CSS" : case "PHP" : case "JS" : case "C" : // Sin estado
                    ...
                    if (substr($Linea, $i, 2) == "/*" && Estado != "HTML") { 
                    ...
                    // Comentario (JS, PHP, C)
                    else if (substr($Linea, $i, 2) == "//" && Estado != "HTML" && $Estado != "CSS") { 
                    ...
                    // Número
                    else if ($this->_EsNumero($Linea[$i]) == true && $this->_BuscarDelimitador($Linea[$i - 1], $Delimitadores) !== false && $Tipo != "CSS" && $Tipo != "HTML") {
                    ...
                    // Posible etiqueta HTML
                        ...
                        if ($this->_BuscarDelimitador($Linea[$i], $Delimitadores) !== false) {
                            ...
                            if (isset($Linea[$i + 1])) {
                                if ($this->_BuscarDelimitador($Linea[$i + 1], $Delimitadores) === false) {
                                    if ($PalabraActual["Texto"] != "") $Palabras[$TotalPalabras ++] = $PalabraActual;
                                    $PalabraActual = array("Texto" => "", "Estado" => $Estado);
                                }
                            }
...
// Función que pinta el string especificado dentro de un marco de código
                        ...
                        foreach ($Diccionario as $Palabra) {				
                            if (isset($Palabra['Palabra'])) {
                                ...
                                if ($PosPalabra !== false) { // El operador !== también puede ser usado. Puesto que != no funcionará como se espera porque si la posición de la palabra es 0. La declaración (0 != false) se evalúa a false.
                                    ...
                                    if ($DelimitadorInicio !== false && $DelimitadorFin !== false) { 
    ...
    private function _BuscarDelimitador($Caracter, &$Delimitadores) {
        foreach($Delimitadores as $Delimitador) {
            if ($Caracter === $Delimitador) { return $Caracter; }
        }
        return false;
    }        
        ...
        foreach ($Diccionario as $Palabra) {		
            if (isset($Palabra['Palabra'])) {
                ...
                if ($PosPalabra !== false) { // El operador !== también puede ser usado. Puesto que != no funcionará como se espera porque si la posición de la palabra es 0. La declaración (0 != false) se evalúa a false.
                    ...
                    // Si la palabra esta bien delimitada la coloreamos
                    if ($DelimitadorInicio !== false && $DelimitadorFin !== false) return $Palabra['Color'];
                }
            }
        }
