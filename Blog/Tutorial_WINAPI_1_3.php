<?php 
	...
	$META = '<meta name="description" content="Tutorial WINAPI">
        ...
        ?>	
                ...
                <p>Como podemos observar a la API <a href="http://msdn.microsoft.com/en-us/library/ms632680%28VS.85%29.aspx" target="_blank">CreateWindowEx</a> le pasamos en  el último parámetro el puntero this, de esta forma al iniciarse la ventana  desde el _GestorMensajes tendremos el puntero a nuestra clase Ventana para poder  enlazarlo definitivamente, y pasarle sus mensajes correctamente a la función  virtual GestorMensajes. Al usar la API <a href="http://msdn.microsoft.com/en-us/library/ms632680%28VS.85%29.aspx" target="_blank">CreateWindowEx</a> estaremos invocando entre  otros el mensaje <a href="http://msdn.microsoft.com/es-es/library/ms632619%28v=VS.85%29.aspx" target="_blank">WM_CREATE</a> para esta ventana, el cual será captado por la  función _GestorMensajes.</p>
                ...
                <p>Esta sería la implementación para nuestro _GestorMensajes  estático, solo debemos hacer dos cosas, interceptar el mensaje <a href="http://msdn.microsoft.com/es-es/library/ms632619(v=VS.85).aspx" target="_blank">WM_CREATE</a> para  obtener el puntero a nuestra clase Ventana y asignarlo definitivamente de forma  que este _GestorMensajes mande los mensajes a su Ventana::GestorMensajes  correspondiente. Para ello utilizamos la API <a href="http://msdn.microsoft.com/en-us/library/ms644898%28VS.85%29.aspx" target="_blank">SetWindowLongPtr</a>, que nos permite  asignar a un área de memoria específica para esta ventana, en el cual asignamos  nuestro puntero a la clase Ventana. </p>
                <p>Más abajo tenemos la sección default, que intentara pasar  todos los mensajes que no sean <a href="http://msdn.microsoft.com/es-es/library/ms632619%28v=VS.85%29.aspx" target="_blank">WM_CREATE</a> a su ventana padre utilizando el  puntero asignado anteriormente a la ventana. Para obtener dicho puntero se  utiliza la API <a href="http://msdn.microsoft.com/en-us/library/ms633585(v=VS.85).aspx" target="_blank">GetWindowLongPtr</a> además de una serie de castings, si resulta que  el resultado es NULL (cosa que no debería) se utilizara la API <a href="http://msdn.microsoft.com/en-us/library/ms633572(v=VS.85).aspx" target="_blank">DefWindowProc</a>  para procesar el mensaje.</p>
                <p>Algunos puede que penséis, porque tanto lio en el <a href="http://msdn.microsoft.com/es-es/library/ms632619%28v=VS.85%29.aspx" target="_blank">WM_CREATE</a>  si podemos hacer el <a href="http://msdn.microsoft.com/en-us/library/ms644898%28VS.85%29.aspx" target="_blank">SetWindowLongPtr</a> justo después de llamar a <a href="http://msdn.microsoft.com/en-us/library/ms632680%28VS.85%29.aspx" target="_blank">CreateWindowEx</a>,  pues esto se ha hecho así debido a que al usar <a href="http://msdn.microsoft.com/en-us/library/ms632680%28VS.85%29.aspx" target="_blank">CreateWindowEx</a> se mandan una  serie de mensajes, a los cuales no tendríamos acceso si utilizamos  <a href="http://msdn.microsoft.com/en-us/library/ms644898%28VS.85%29.aspx" target="_blank">SetWindowLongPtr</a> al terminar <a href="http://msdn.microsoft.com/en-us/library/ms632680%28VS.85%29.aspx" target="_blank">CreateWindowEx</a>, y esto nos puede traer problemas  extra en el primer pintado de la ventana, y con alguna cosa más.</p>
